...
This extension is installed directly from source — it takes about 2 minutes. No account or payment required. Works on Chrome, Edge, Brave, and other Chromium-based browsers.

### Step 1: Download the files

Go to the [latest release](https://github.com/yourusername/argos-in-stock-filter/releases/latest) and download the ZIP file from the "Assets" section.
...
1. Download the new ZIP from the [releases page](https://github.com/yourusername/argos-in-stock-filter/releases)
...
On the GitHub repo page, click **Watch** → **Custom** → tick **Releases**. You'll get an email when a new version is published.
...
If Argos changes their DOM, edit `oosSelector` in `config.js`:
...
| `generate_icons.py` | One-time icon generator (stdlib only, no Pillow) |
